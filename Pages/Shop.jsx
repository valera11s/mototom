...
export default function Shop() {
  ...
  useEffect(() => {
    setCategoryKey(resolveCategoryKey(params.get('category')));
  }, [params]);
  ...
  const full = useMemo(
    () => source.map((item, idx) => ({
      ...item,
      subtype: inferSubtype(item, activeCategory, idx),
  ...
  const availableSubtypes = useMemo(
    () => unique([...activeCategory.subtypes, ...full.map((item) => item.subtype)]),
    [activeCategory, full]
  );
  ...
  const shopStructuredData = useMemo(() => {
    ...
    return {
      ...
      '@type': 'CollectionPage',
      name: `Каталог: ${activeCategory.label}`,
      description: `Каталог мотоэкипировки MOTOTOM в категории «${activeCategory.label}».`,
  ...
  }, [activeCategory.label, filtered, products]);

  return (
    <div className="mx-auto w-full max-w-[1440px] pb-12 text-slate-100">
      <Seo
        title={`${activeCategory.label} — каталог мотоэкипировки`}
        description={`Купить ${activeCategory.label.toLowerCase()} в MOTOTOM. ${filtered.length} товаров с ценами, фильтрами и быстрой доставкой.`}
      ...
      <div className="grid grid-cols-1 gap-6 px-4 md:px-12 lg:grid-cols-[260px_1fr] lg:gap-8">
        <aside className="space-y-8">
          ...
          <div className={`${showAllFilters ? 'block' : 'hidden'} space-y-5 lg:block`}>
            <>
              <div className="space-y-3.5 border-t border-[#1E1E22] pt-4">
                <p className="text-sm font-semibold text-[#FAFAF9]">{activeCategory.subtypeTitle}</p>
                {availableSubtypes.map((type) => (
                  <button key={type} type="button" onClick={() => toggle(type, selectedSubtypes, setSelectedSubtypes)} className="flex w-full items-center gap-2.5 text-left text-[13px] text-[#A0A0A5]">
