-- Скрипт для добавления товаров с брендом DJI
-- Категория: Фотоаппараты (ID = 331)
-- Подкатегория: Зеркальные фотоаппараты (ID нужно указать)

-- Сначала узнайте ID подкатегории выполнив:
-- SELECT id, name, parent_id FROM categories WHERE parent_id = 331;
...
-- 2. Замените <SUBCATEGORY_ID> на реальный ID подкатегории "Зеркальные фотоаппараты"
-- Или выполните весь блок DO:

DO $$
DECLARE
    v_category_id INTEGER := 331;  -- Фотоаппараты
    v_subcategory_id INTEGER;
BEGIN
    -- Находим подкатегорию "Зеркальные фотоаппараты" с parent_id = 331
    SELECT id INTO v_subcategory_id 
    FROM categories 
    WHERE name = 'Зеркальные фотоаппараты' AND parent_id = 331 
    LIMIT 1;
    
    IF v_subcategory_id IS NULL THEN
        RAISE EXCEPTION 'Подкатегория "Зеркальные фотоаппараты" не найдена для категории ID=331.';
    END IF;
